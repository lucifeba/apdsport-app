...
const replaceOnce = (before, after, appliedMarker, label) => {
  if (code.includes(appliedMarker)) return;
...
replaceOnce(
  ...
  'Todos los arrays deben contener solo strings',
  'AI output contract',
);
...
if (!code.includes('async function notifyFailure')) {
...
console.log('Production hardening patch ready');
